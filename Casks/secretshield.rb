...
cask "secretshield" do
  ...
  caveats <<~EOS
    ...
    Homebrew ставит каски с меткой карантина, а приложение подписано
    самодельным сертификатом, а не сертификатом Apple (99 долларов в год,
    пока не куплен). Пока метка на месте, macOS не даёт запустить приложение
    напрямую — а значит не работает и автозапуск при входе в систему.

    Дальше приложению нужен Универсальный доступ (Accessibility): без него
    нельзя остановить вставку. При первом запуске оно само откроет нужный
    раздел настроек: System Settings → Privacy & Security → Accessibility.

    Разрешение переживает обновления: подпись у сборок одна и та же.
  EOS
end
